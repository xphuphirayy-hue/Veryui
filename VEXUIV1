...
function UIFramework:EnableTabSystem(window)
	...
	--------------------------------------------------------
	-- Override ฟังก์ชัน CreateTab ของ Window
	--------------------------------------------------------
...
function UI:CreateFloatingButton(window)
    ...
    button.Image = "rbxassetid://7072719064"  -- ไอคอนเมนู
...
function UI:EnableResize(window)
    ...
    grip.Image = "rbxassetid://7072706622" -- ไอคอนมุม (…)
...
--// ฟังก์ชันใช้บันทึกค่าจาก element UI
...
---------------------------------------------------------------------
-- PART 13 — Search Bar System (Full)
---------------------------------------------------------------------
...
--// ฟังก์ชันเรียกใช้เมื่อสร้าง Sidebar
...
---------------------------------------------------------------------
-- PART 14 — Tooltip System (Full)
---------------------------------------------------------------------

-- สร้าง Tooltip หนึ่งอันสำหรับทั้ง Library
...
-- อัพเดตตำแหน่ง tooltip ตามเมาส์
UserInputService.InputChanged:Connect(function(input)
...
-- ฟังก์ชันใช้ผูก Tooltip ให้ element ใดก็ได้
...
---------------------------------------------------------------------
-- PART 15 — DropShadow Pro (Full)
---------------------------------------------------------------------

-- ฟังก์ชันสร้าง Shadow สำหรับ UI Element ใดก็ได้
function Library:AddShadow(parent, intensity)
    intensity = intensity or 0.55  -- ความเข้มของเงา (0.3–0.7 แนะนำ)
    ...
    -- อัพเดตเงาตามขนาด Element (รองรับ Resize)
...
-- ฟังก์ชันสร้าง Tween พร้อม Preset
...
------------------------------------------------------------
-- 🎯 Preset 3: Back (เด้งกลับเล็กน้อย)
------------------------------------------------------------
...
------------------------------------------------------------
-- 🎯 Preset 4: Spring (เหมือน Discord)
------------------------------------------------------------
...
UISound.Sounds = {
    Hover = "rbxassetid://9118822716",       -- เสียงชี้เมาส์
    Click = "rbxassetid://7149516992",       -- เสียงคลิก
...
PasswordManager.Opened = {} -- เก็บว่าแท็บไหนปลดล็อกแล้ว
...
-- Button Hover (ขยายเล็กน้อย + ความสว่าง)
...
-- ถ้าใส่ Key ถูก จะไม่ถามอีกภายในรอบนี้
...
-- ตัว Blur ที่จะใช้กับ Workspace
...
--// Smooth Depth Blur (หน้าใกล้/ไกล) ---------------
function BlurEngine.Depth(UI, Depth)
    -- Depth: 0 (ชัด) → 1 (เบลอมาก)
...
function Parallax.Apply(UI, Strength)
    Strength = Strength or 15  -- ยิ่งมากเอียงยิ่งเยอะ
    ...
    local function tilt(input)
        ...
        CurrentTween = TweenService:Create(UI, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
            Rotation = RotY,   -- หมุนรอบ Z (เอียงซ้าย/ขวา)
        })
        CurrentTween:Play()

        -- เพิ่มเอียงแบบ 3D ด้วย UIGradient หรือ UICorner ก็ทำได้เพิ่มภายหลัง
    end

    -- เมื่อเอาเมาส์เข้า UI
    UI.MouseEnter:Connect(function()
        Hovering = true
    end)

    -- เมื่อออก UI → รีเซ็ตมุม
    UI.MouseLeave:Connect(function()
        Hovering = false
...
ThemeEngine.Themes = {
    ...
    Rainbow = {
        ...
        Accent     = nil, -- ระบบนี้จะเรนโบว์เอง
        Text       = Color3.fromRGB(255, 255, 255)
    }
}
...
function GlassEngine.Apply(UI, Settings)
    ...
    -- 5) Background Blur — ใช้ BlurService จาก PART 26
...
-- เก็บค่าสีปัจจุบันของ UI
UISettings = UISettings or {}
UISettings.PrimaryColor = UISettings.PrimaryColor or Color3.fromRGB(0, 170, 255)

-- รายชื่อ UI ทั้งหมดที่ลงทะเบียนให้เปลี่ยนสีพร้อมกัน
local ColorUpdateCallbacks = {}

-- ฟังก์ชันลงทะเบียน UI ให้รับการอัปเดตสี
function BindColorUpdate(fn)
    table.insert(ColorUpdateCallbacks, fn)
    fn(UISettings.PrimaryColor) -- อัปเดตสีทันทีตอนสมัคร
end

-- ฟังก์ชันเปลี่ยนสีหลักแบบ Real-time
...
-- ตัวอย่างการ Bind UI ทั้งหมดในระบบ (ให้เปลี่ยนสีพร้อมกัน)
...
---------------------------------------------------------
--// PART 32 — Tabs + SubTabs System (Nested Tabs)
---------------------------------------------------------
...
function CreateTab(TabName)
    ...
    -- เมื่อคลิก Tab จะซ่อน tab อื่นทั้งหมด
...
---------------------------------------------------------
--// PART 35 — Button Groups (Radio / Multi Select)
---------------------------------------------------------

function CreateButtonGroup(parent, title, options, mode, callback)
    -- mode: "single" หรือ "multi"
    local Group = {}
...
---------------------------------------------------------
--// PART 36 — Toggle with Indicator Light
---------------------------------------------------------
...
---------------------------------------------------------------------
-- PART 38 — Keybind Recorder + Display
---------------------------------------------------------------------
...
---------------------------------------------------------------------
-- PART 39 — Modal Windows (Confirm / Alert / Warning / Input Prompt)
---------------------------------------------------------------------
...
---------------------------------------------------------------------
-- PART 40 — Auto Update UI (โหลดเวอร์ชันใหม่จาก GitHub)
---------------------------------------------------------------------
...
function UI:CheckUpdate(cfg)
    ...
    if remoteVersion > currentVersion then
        UI:Modal({
            ...
            Content = "มีเวอร์ชันใหม่ " .. remoteVersion .. " (ปัจจุบัน " .. currentVersion .. ")\nต้องการอัปเดตหรือไม่?",
            Button1 = "Update",
            Button2 = "Cancel",
            Callback = function()
                ...
                UI:Notification({
                    Title = "Updated!",
                    Content = "ดาวน์โหลดเสร็จแล้ว รีสคริปเพื่อใช้งานเวอร์ชันล่าสุด",
                    Duration = 5
                })
            end
        })
...
---------------------------------------------------------------------
-- PART 42 — Local Notifications Center (ประวัติแจ้งเตือน)
---------------------------------------------------------------------
...
function UI:OpenNotificationCenter()
    ...
    for i, n in ipairs(UI.NotificationHistory) do
        ...
        item.Text = "["..n.Time.."] "..n.Title.." — "..n.Content
...
---------------------------------------------------------------------
-- PART 43 — FPS / Ping Monitor Widget
---------------------------------------------------------------------
...
---------------------------------------------------------------------
-- PART 44 — UI Safe Mode (ลดแอนิเมชันเพิ่ม FPS)
---------------------------------------------------------------------
UI.SafeMode = false
...
function UI:SetSafeMode(state)
    ...
    if state then
        ...
        UI:Notification({
            Title = "Safe Mode",
            Content = "ปิดเอฟเฟกต์บางอย่างเพื่อเพิ่ม FPS",
            Duration = 3
        })
    else
...
--========================================================
-- PART 45 : UI CONFIG SAVE / LOAD (XENO EXECUTOR)
--========================================================

local ConfigSystem = {}
local HttpService = game:GetService("HttpService")

local CONFIG_KEY = "VEX_UI_CONFIG"

-- Save Config สำหรับ Xeno Executor
function ConfigSystem:Save(configTable)
    local json = HttpService:JSONEncode(configTable)

    if Xeno and Xeno.Set then
        Xeno.Set(CONFIG_KEY, json)
        print("[VEX UI] Config saved via Xeno")
    else
        warn("[VEX UI] Xeno.Set not available")
    end
end

-- Load Config สำหรับ Xeno Executor
function ConfigSystem:Load()
    if Xeno and Xeno.Get then
        local json = Xeno.Get(CONFIG_KEY)
        if json and #json > 0 then
            local success, data = pcall(function()
                return HttpService:JSONDecode(json)
            end)
            if success then
                print("[VEX UI] Config loaded via Xeno")
                return data
            else
                warn("[VEX UI] Failed to decode Xeno config JSON")
            end
        end
    end
    return {}
...
function ConfigSystem:Apply(UI, config)
    if not config then return end

    if config.Position and UI.SetPosition then
        UI:SetPosition(config.Position)
    end

    if config.Size and UI.SetSize then
        UI:SetSize(config.Size)
    end

    if config.Theme and UI.SetTheme then
...
--========================================================
-- PART 46 : AUTO LOAD UI CONFIG ON START
--========================================================

function ConfigSystem:AutoLoad(UI)
    local cfg = self:Load()
    if cfg and next(cfg) ~= nil then
        self:Apply(UI, cfg)
        print("[VEX UI] Auto-applied saved config")
    else
        print("[VEX UI] No saved UI config to load")
    end
end

--========================================================
-- PART 46 : THEME MANAGER
--========================================================
...
function ThemeManager:SetTheme(UI, themeName)
    ...
    print("[ThemeManager] Theme applied:", themeName)
end

return ThemeManager

--========================================================
-- PART 47 : DRAG + RESIZE SYSTEM
--========================================================

local DragResize = {}
local UIS = game:GetService("UserInputService")

function DragResize:EnableDrag(frame)
    local dragging = false
    local dragStart, startPos
...
function DragResize:EnableResize(frame, sizeHandle)
    local dragging = false
...
--========================================================
-- PART 48 : KEYBIND SYSTEM
--========================================================

local KeybindSystem = {}
local UIS = game:GetService("UserInputService")

KeybindSystem.CurrentKey = Enum.KeyCode.RightShift -- default

function KeybindSystem:SetKey(newKey)
    self.CurrentKey = newKey
    print("[Keybind] Changed key to:", newKey.Name)
end

function KeybindSystem:BindToggle(UI)
    UIS.InputBegan:Connect(function(input)
        if input.KeyCode == self.CurrentKey then
...
--========================================================
-- PART 49 : ANIMATION ENGINE
--========================================================

local Animation = {}
local TweenService = game:GetService("TweenService")

function Animation:Tween(obj, time, style, goal)
    local tween = TweenService:Create(obj, TweenInfo.new(
        time,
        Enum.EasingStyle[style] or Enum.EasingStyle.Sine,
        Enum.EasingDirection.Out
    ), goal)
    tween:Play()
    return tween
...
function Animation:Fade(obj, duration, alpha)
    return self:Tween(obj, duration, "Sine", {BackgroundTransparency = alpha})
end

function Animation:SlideX(obj, duration, offset)
    return self:Tween(obj, duration, "Quad", {Position = obj.Position + UDim2.new(0, offset, 0, 0)})
end

function Animation:SlideY(obj, duration, offset)
    return self:Tween(obj, duration, "Quad", {Position = obj.Position + UDim2.new(0, 0, 0, offset)})
end

return Animation

--========================================================
-- PART 50 : NOTIFICATION SYSTEM
--========================================================

local Notification = {}
local TweenService = game:GetService("TweenService")

function Notification:Create(text)
...
local TabMemory = {}
local HttpService = game:GetService("HttpService")
local KEY = "VEXUI_LAST_TAB"

function TabMemory:Save(tabName)
    if Xeno and Xeno.Set then
        Xeno.Set(KEY, tabName)
    else
        getgenv()[KEY] = tabName
    end
end

function TabMemory:Load()
    if Xeno and Xeno.Get then
        return Xeno.Get(KEY)
    else
        return getgenv()[KEY]
...
function TabMemory:Apply(UI, tabs)
    local last = self:Load()
    if last and tabs[last] then
        for name, tab in pairs(tabs) do
            tab.Visible = (name == last)
        end
        print("[VEXUI] Loaded last tab:", last)
    end
end
...
function BlurSystem:SetTransparent(UI, alpha)
    for _, obj in pairs(UI:GetDescendants()) do
        if obj:IsA("Frame") or obj:IsA("TextLabel") or obj:IsA("TextButton") then
...
function BlurSystem:SetBlur(enable)
    blur.Size = enable and 18 or 0
end

return BlurSystem

--========================================================
-- PART 53 : CUSTOM CURSOR SYSTEM
--========================================================

local CursorSystem = {}

function CursorSystem:Enable(cursorId)
    game:GetService("UserInputService").MouseIconEnabled = true
    game:GetService("UserInputService").MouseIcon = cursorId
end

function CursorSystem:Disable()
...
--========================================================
-- PART 54 : IMPORT / EXPORT CONFIG
--========================================================

local ImportExport = {}
local HttpService = game:GetService("HttpService")
local KEY = "VEX_UI_CONFIG"

function ImportExport:Export(configTable)
    local json = HttpService:JSONEncode(configTable)
    local base64 = HttpService:Base64Encode(json)
    return base64
end

function ImportExport:Import(base64)
    local decoded = HttpService:Base64Decode(base64)
    local success, data = pcall(function()
    ...
    if success then
        ...
        warn("[VEX UI] Failed to import config")
        return {}
    end
end
...
--========================================================
-- PART 55 : MULTI-PROFILE SYSTEM
--========================================================

local Profile = {}
local HttpService = game:GetService("HttpService")

function Profile:SetProfile(name)
    getgenv().VEX_PROFILE = name
end

function Profile:GetProfile()
    return getgenv().VEX_PROFILE or "Default"
end

function Profile:GetProfileKey()
    return "VEX_UI_PROFILE_" .. self:GetProfile()
end

function Profile:Save(data)
    local key = self:GetProfileKey()
    Xeno.Set(key, HttpService:JSONEncode(data))
end

function Profile:Load()
    local key = self:GetProfileKey()
    local raw = Xeno.Get(key)
    if raw then
...
function Tooltip:Attach(obj, text)
    ...
    game:GetService("UserInputService").InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement then
            tip.Position = UDim2.new(0, input.Position.X + 10, 0, input.Position.Y + 10)
        end
    end)
end
...
--========================================================
-- PART 57 : UI FPS OPTIMIZER
--========================================================

local FPSOptimizer = {}

function FPSOptimizer:Optimize(UI)
    UI:GetPropertyChangedSignal("Visible"):Connect(function()
        if UI.Visible == false then
            setfpscap(30)   -- ลดโหลดตอนปิด UI
        else
            setfpscap(120)  -- กลับมาปกติ
        end
    end)
end
...
--========================================================
-- PART 58 : PERMISSION SYSTEM
--========================================================

local Permission = {}

function Permission:Check(isDev)
    getgenv().VEX_IS_DEV = isDev
end

function Permission:Apply(tabs)
    for name, tab in pairs(tabs) do
        if tab.DevOnly and not getgenv().VEX_IS_DEV then
            tab.Visible = false
        end
    end
end
...
function RemoteTheme:Load(url, UI)
    local raw = game:HttpGet(url)
    local themeTable = HttpService:JSONDecode(raw)
    ThemeManager.Themes[themeTable.Name] = themeTable
    ThemeManager:SetTheme(UI, themeTable.Name)
end

return RemoteTheme

--========================================================
-- PART 60 : NETWORK CONNECTOR
--========================================================
...
function Net:GetMessage(url)
    local data = self:GetJSON(url)
...
--========================================================
-- PART 61 : REAL TIME CLOCK (UI LEFT-BOTTOM)
--========================================================

local RealClock = {}

function RealClock:AddClock(UI)
    local clock = Instance.new("TextLabel")
    clock.Parent = UI
    clock.AnchorPoint = Vector2.new(0, 1)
    clock.Position = UDim2.new(0, 10, 1, -10)   -- มุมซ้ายล่าง
    clock.Size = UDim2.new(0, 180, 0, 25)
...
----------------------------------------------------------
-- PART 62 : AUTO DAY/NIGHT THEME
----------------------------------------------------------

function Addons.AutoTheme(UI, ThemeManager)
    task.spawn(function()
        while true do
            local hour = tonumber(os.date("%H"))
            if hour >= 6 and hour < 18 then
                ThemeManager:SetTheme(UI, "Light")
            else
                ThemeManager:SetTheme(UI, "Dark")
            end
            task.wait(60)
        end
    end)
...
----------------------------------------------------------
-- PART 64 : PARALLAX PANEL (3D EFFECT)
----------------------------------------------------------

function Addons.Parallax(frame)
    UIS.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement then
            local x = (input.Position.X / 1920) - 0.5
            local y = (input.Position.Y / 1080) - 0.5
            frame.Rotation = x * 8
            frame.Position = frame.Position + UDim2.new(0, x * 3, 0, y * 3)
        end
    end)
end

----------------------------------------------------------
-- PART 65 : GRADIENT ENGINE
----------------------------------------------------------

function Addons.Gradient(obj, c1, c2)
    local g = Instance.new("UIGradient", obj)
    g.Color = ColorSequence.new(c1, c2)
end

----------------------------------------------------------
-- PART 66 : AMBIENT SOUND PLAYER
----------------------------------------------------------

function Addons.PlayAmbient(soundId)
...
function Addons.ClickSound(obj, soundId)
    obj.MouseButton1Click:Connect(function()
        local s = Instance.new("Sound", obj)
        s.SoundId = soundId
        s.Volume = 0.3
        s:Play()
    end)
...
----------------------------------------------------------
-- PART 67 : ANIMATED TAB ICONS (PULSE EFFECT)
----------------------------------------------------------

function Addons.TabPulse(icon)
    task.spawn(function()
        while icon.Parent do
            icon.Size = icon.Size + UDim2.new(0, 2, 0, 2)
            task.wait(0.2)
            icon.Size = icon.Size - UDim2.new(0, 2, 0, 2)
            task.wait(0.2)
        end
...
----------------------------------------------------------
-- PART 68 : ANTI-OVERLAP SYSTEM
----------------------------------------------------------

function Addons.AntiOverlap(list)
    for i, uiA in ipairs(list) do
        for j, uiB in ipairs(list) do
            if i ~= j then
                if uiA.AbsolutePosition.X < uiB.AbsolutePosition.X + uiB.AbsoluteSize.X and
                   uiA.AbsolutePosition.X + uiA.AbsoluteSize.X > uiB.AbsolutePosition.X and
                   uiA.AbsolutePosition.Y < uiB.AbsolutePosition.Y + uiB.AbsoluteSize.Y and
                   uiA.AbsolutePosition.Y + uiA.AbsoluteSize.Y > uiB.AbsolutePosition.Y then

                    uiB.Position = uiB.Position + UDim2.new(0, 25, 0, 25)
                end
            end
        end
    end
...
----------------------------------------------------------
-- PART 69 : FULLSCREEN BUTTON
----------------------------------------------------------

function Addons.Fullscreen(UI)
    local btn = Instance.new("TextButton", UI)
    btn.Size = UDim2.new(0, 50, 0, 25)
    btn.Position = UDim2.new(1, -60, 0, 10)
    btn.Text = "⛶"
    ...
    btn.MouseButton1Click:Connect(function()
        if not isFull then
            ...
            UI.Position = UDim2.new(0, 25, 0, 25)
            isFull = true
        else
            UI.Size = oldSize
            UI.Position = oldPos
